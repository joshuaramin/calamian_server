                ...
                switch (dmy) {
                    ...
                    `) as any
                        ...
                        return weekly.map(({ period, record_count }) => {
                            return { date: format(new Date(period), "MMMM dd, yyyy"), total: parseInt(record_count) }
                        })
                    case "Monthly":
                    ...
                    `) as any
                        ...
                        return monthly.map(({ period, record_count }) => {
                            return { date: format(new Date(period), "MMMM"), total: parseInt(record_count) }
                        })
                    case "Yearly":
                    ...
                    ORDER BY
                        ...
                        return yearly.map(({ period, record_count }) => {
                            return { date: format(new Date(period), "yyyy"), total: parseInt(record_count) }
                        })
                    default:
                        throw new GraphQLError("Please selecet on the given enum, Weekly, Monthly, and Yearly")
                }
            }
        })
